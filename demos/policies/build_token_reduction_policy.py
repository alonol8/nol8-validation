...
OUTPUTS = {
    "conservative": HERE / "token-reduction.nol",
    "aggressive": HERE / "token-reduction-aggressive.nol",
}
...
def check_safe(pairs: list[tuple[str, str]]) -> None:
    ...
    for literal, replacement in pairs:
        ...
        # Delimiting is what makes a rule safe, not length. " an " cannot fire
        # inside "another" or "an­alysis" because both spaces have to be there;
        # an undelimited "an" would fire inside both. Short function words are
        # therefore fine, and they are most of the saving.
...
def render(pairs: list[tuple[str, str]], profile: str) -> str:
    lines = [
        f"# Token-reduction policy ({profile}) - shrink text before it is billed",
    ...
    if profile == "aggressive":
        lines.extend([
            "# Includes function words, which are where the volume is. Output is",
            "# no longer readable prose - this profile is for text headed to an",
            "# embedding pipeline, not to a person. Negations are deliberately",
            "# kept: dropping \"not\" inverts the meaning of the sentence.",
        ])
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="both",
        help="conservative keeps the output readable; aggressive also removes "
             "function words, which is where most of the saving is",
    )
    parser.add_argument(
        "--show-sample",
        action="store_true",
    ...
    profiles = (
        ("conservative", "aggressive") if args.profile == "both" else (args.profile,)
    )
    for profile in profiles:
        pairs = reduction_rules(profile)
        check_safe(pairs)
        output = OUTPUTS[profile]
        output.write_text(render(pairs, profile))
        ...
        extra = f", {len(FUNCTION_WORDS)} function words" if profile == "aggressive" else ""
        print(f"Wrote {output.name}: {len(pairs)} rules "
              f"({len(PLAINER_PHRASES)} phrases, {len(PLAINER_WORDS)} words, "
              f"{len(STOPWORDS)} removed{extra}).")
