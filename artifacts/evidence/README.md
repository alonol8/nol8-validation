...
| `corpus-density-20260726.{json,md}` | offline, oracle over the sweep corpora | Match density per rule count: **flat 0.120–0.125 matches/KB, no rise with rule count** → the trend is a ruleset-size effect, not a density artifact. Also carries Alon's corpus coordinates (distinct bodies, diversity, avg bytes, near-misses/KB). |
