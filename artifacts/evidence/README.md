...
| `rulecount-2k4k6k8k-clean-20260726.csv` | 10 edge, conc 256, small, 5 reps | **Single-run 4-point trend (median basis).** Themis flat 75.5–76.2k (1.9% span across 2k→8k); Aergia declines 59.3k→57.3k→56.1k→55.9k (−5.7%, plateaus by 6k). Ratios 1.276/1.323/1.347/1.362×. 8k reproduces the prior 10-edge 1.346×. **Supersedes** `rulecount-2k4k-clean-20260726.csv` and `rulecount-10argus-clean.csv` as the trend basis. |
| `large-rerun-4k-20260726.csv` | 10 edge, conc 256, 4k rules, large, 3 reps | Clean-ish large re-run. Median 1.05× (Themis 1127 / Aergia 1074); ~306–323 MiB/s both. Themis had one 0-error rep (1128 rps, 322.96 MiB/s); Aergia contaminated every rep (58–87 err). Large ratio stays provisional. |
| `corpus-density-sent-20260726.json` + `corpus-density-20260726.{json,md}` | offline, oracle over the sweep corpora | Match density. **The SENT small-band bodies (what the driver scanned): ~4.8 matches/KB, ~11 matches/request, ~74% hit — flat across rule counts** → trend is a ruleset-size effect, not density (findings 009; Alon's ~5/KB estimate was right). The `-20260726.json` source-doc figure (0.12/KB over 115 KB docs) is the WRONG object — context only, do not quote. The `.md` explains both. |
| `efficiency-idle-20260726.csv` | engine hosts, 5 reps, idle | Cores per data-plane process, 5 reps + spread. Themis apollo median 11.17; Aergia apollo 11.27 + lexers 8.17 = 19.44 total; **~8.3-core software tax.** Idle (poll-mode); under-load confirmation pending (findings 009 item 5 → `efficiency-under-load-20260726.csv`). |
| `efficiency-constants.json` | derived from the CSVs | **SINGLE SOURCE OF TRUTH** for efficiency cores + throughput (console reads it; no more hardcoded copies). cores_per_1k / ratio left null pending the under-load confirmation; console derives a provisional ~2.37× for display. |
